...
import { CommunityList } from "./community-details";
...
//   createCommunityModalOpened: boolean;
//   editCommunityModalOpened: boolean;
...
const CreateCommunityModal: Component<CreateCommunityModalProps> = (props) => {
  const [Title, setName] = createSignal("");
  ...
  const filled = createMemo(() => {
    debugger
    const teamDetailsFilled = Title() && description() && skills().length > 0;
...
const EditTeamModal: Component<EditCommunityModalProps> = (props) => {
  ...
  const editCommunity = async () => {
    // await authenticatedAPI.community.updateCommunity({
  ...
  //   await authenticatedAPI.community.removeTeamMember({
  //     communityId: props.communityId,
...
interface CommunityPageModalsProps {
  createCommunityModalOpened: boolean;
  editCommunityModalOpened: boolean;
  setCreateCommunityModalOpened: Setter<boolean>;
  setEditCommunityModalOpened: Setter<boolean>;
  selectedTeam: Schemas["TeamListDto"];
}
const TeamsPageModals: Component<CommunityPageModalsProps> = (props) => {
  const authenticatedAPI = useAuthenticatedAPI();
  ...
  return (
    <>
      <CreateCommunityModal
        opened={props.createCommunityModalOpened}
        setOpened={props.setCreateCommunityModalOpened}
        profile={profile()!}
      />
      {/* <EditTeamModal
        opened={props.editCommunityModalOpened}
        teamId={props.selectedTeam.teamId}
        initialData={props.selectedTeam}
        setOpened={props.setEditCommunityModalOpened}
        profile={profile()!}
      ></EditCommunityModal> */}
    </>
  );
...
const CommunityPage: Component = () => {
  const [createCommunityModalOpened, setCreateCommunityModalOpened] = createSignal(false);
  const [editCommunityModalOpened, setEditCommunityModalOpened] = createSignal(false);
      ...
      <Suspense fallback={<Loader />}>
        <div class="flex-1 h-[calc(100%-4rem)] overflow-auto">
          <div class="flex flex-col items-center justify-center px-8 pt-4 md:flex-row">
            <div class="flex-1 w-full pb-4 text-center md:text-left md:pb-0">
              <h1 class="w-full text-4xl font-extrabold"></h1>
          ...
          <div class="flex flex-col flex-1 h-[calc(100%-5rem)] p-4 md:p-8">
            <div class="flex items-center justify-center pb-4">
              <IconButton
                ...
                icon={mdiPlusCircle}
                color="primary"
                onClick={() => setCreateCommunityModalOpened(true)}
                label={<span class="pl-2 pr-1">Create Community</span>}
              />
              <div class="flex-1" />
            </div>
            {/* <CommunityList
              createCommunityModalOpened={createCommunityModalOpened()}
              editTeamModalOpened={editCommunityModalOpened()}
              onClick={(community) => {
                const transformedVal = {
                  ...community,
                  name: community.topicId,
                  views:community.views,
                  title:community.title,
                  activity:community.lastPostedAt,
                  likeCount:community.likeCount
                  // title={data.title}
                  // reply={data.replyCount}
                  // activity={data.lastPostedAt}
                ...
                setEditCommunityModalOpened(true);
              }}
            /> */}
          </div>
        </div>
        <div class="flex-col items-center justify-start hidden h-full p-4 ml-2 bg-white border-gray-200 shadow-inner lg:flex w-72">
          ...
          <Button
            color="primary"
            size="large"
            class="mt-8 shadow-lg shadow-[#4A0289]"
            disabled
          >
            Add new goal
          </Button>
        ...
        {/* <CommunityPageModals
          ...
          setCreateTeamModalOpened={setCreateCommunityModalOpened}
          setEditTeamModalOpened={setEditCommunityModalOpened}
        /> */}
      </Suspense>
    </AuthenticatedLayout>
  );
